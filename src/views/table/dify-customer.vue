...
const handleRestNameCom = (item: any) => {
  ...
  request
    .put(resetCustomerApi + item.conversation_id + "/name/", {
      names: item.names,
...
const handleRate = (dataS: any) => {
  ...
  request
    .post(feedbacksCustomerApi + dataS.message_id + "/feedbacks", {
...
const stopFunction = (id: any) => {
  request
    .post(
      stopCustomerApi + id + "/stop?user=" + localStorage.getItem("s_name"),
      {
        user: localStorage.getItem("s_name"),
...
const fetchStreamData = () => {
  ...
  restSender()
  let flag = true
  fetchEventSource(runCustomerApi, {
    method: "POST",
    headers: {
