...
const handleRestNameCom = (item: any) => {
  ...
  request
    .put(resetHealthApi + item.conversation_id + "/name/", {
      names: item.names,
...
const handleRate = (dataS: any) => {
  ...
  request
    .post(feedbacksHealthApi + dataS.item.message_id + "/feedbacks", {
...
const stopFunction = (id: any) => {
  request
    .post(
      stopHealthApi + id + "/stop?user=" + localStorage.getItem("s_name"),
      {
        user: localStorage.getItem("s_name"),
