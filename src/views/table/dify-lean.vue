...
const handleRestNameCom = (item: any) => {
  ...
  request
    .put(resetLeanApi + item.conversation_id + "/name/", {
      names: item.names,
...
const handleRate = (dataS: any) => {
  ...
  request
    .post(feedbacksLeanApi + dataS.message_id + "/feedbacks", {
...
const stopFunction = (id: any) => {
  request
    .post(
      stopLeanApi + id + "/stop?user=" + localStorage.getItem("s_name"),
      {
        user: localStorage.getItem("s_name"),
