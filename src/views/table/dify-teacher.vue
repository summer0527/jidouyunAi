      ...
      >
        <div style="position: fixed; bottom: 60px; width: 60%">
          <el-form :inline="true" class="demo-form-inline">
            <el-form-item label="专业" class="is-required">
              <el-select
                v-model="sqlType"
                placeholder="请选择"
                clearable
...
const handleRestNameCom = (item: any) => {
  ...
  request
    .put(resetTeachingApi + item.conversation_id + "/name/", {
      names: item.names,
...
const handleRate = (dataS: any) => {
  ...
  request
    .post(feedbacksTeachingApi + dataS.message_id + "/feedbacks", {
...
const stopFunction = (id: any) => {
  request
    .post(
      stopTeachingApi + id + "/stop?user=" + localStorage.getItem("s_name"),
      {
        user: localStorage.getItem("s_name"),
...
const fetchStreamData = () => {
  ...
  restSender()
  let flag = true
  fetchEventSource(runLTeachingApi, {
    method: "POST",
    headers: {
