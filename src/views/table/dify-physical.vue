...
const handleRestNameCom = (item: any) => {
  ...
  request
    .put(resetPhysicalApi + item.conversation_id + "/name/", {
      names: item.names,
...
const handleRate = (dataS: any) => {
  ...
  request
    .post(feedbacksPhysicalApi + dataS.message_id + "/feedbacks", {
...
const stopFunction = (id: any) => {
  request
    .post(
      stopPhysicalApi + id + "/stop?user=" + localStorage.getItem("s_name"),
      {
        user: localStorage.getItem("s_name"),
...
const fetchStreamData = () => {
  ...
  restSender()
  let flag = true
  fetchEventSource(runPhysicalApi, {
    method: "POST",
    headers: {
