...
const handleRestNameCom = (item: any) => {
  ...
  request
    .put(resetClassApi + item.conversation_id + "/name/", {
      names: item.names,
...
const handleRate = (dataS: any) => {
  ...
  request
    .post(feedbacksClassApi + dataS.message_id + "/feedbacks", {
...
const stopFunction = (id: any) => {
  request
    .post(
      stopClassApi + id + "/stop?user=" + localStorage.getItem("s_name"),
      {
        user: localStorage.getItem("s_name"),
