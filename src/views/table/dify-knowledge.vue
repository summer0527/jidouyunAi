      ...
      >
        <div style="position: fixed; bottom: 60px; width: 60%">
          <el-form :inline="true" class="demo-form-inline">
            <el-form-item label="行业" class="is-required">
              <el-select
                v-model="sqlType"
                placeholder="请选择"
                clearable
...
const sqlType = ref("测绘行业");
const sqlTypeOptions = ref([
{ label: "测绘行业", value: "测绘行业" },
  { label: "汽车行业", value: "汽车行业" },
  { label: "飞机行业", value: "飞机行业" },
]);
...
const handleRestNameCom = (item: any) => {
  ...
  request
    .put(resetknowledgeApi + item.conversation_id + "/name/", {
      names: item.names,
...
const handleRate = (dataS: any) => {
  ...
  request
    .post(feedbacksknowledgeApi + dataS.message_id + "/feedbacks", {
...
const stopFunction = (id: any) => {
  request
    .post(
      stopknowledgeApi + id + "/stop?user=" + localStorage.getItem("s_name"),
      {
        user: localStorage.getItem("s_name"),
...
industry: sqlType.value,
  ...
  restSender()
  let flag = true
  fetchEventSource(runLknowledgeApi, {
    method: "POST",
    headers: {
